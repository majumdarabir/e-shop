const likeReview = async ({ review }) => {
  ...
  const unlikeButtonElement = document.getElementById(
    `${review}-unlike-button`
  );

  try {
    const request = await fetch(`/like-review/${review}`, {
      method: "GET",
    });
...
const unlikeReview = async ({ review }) => {
  ...
  try {
    const request = await fetch(`/unlike-review/${review}`, {
      method: "GET",
    });
    const data = await request.json();
    console.log(data);
